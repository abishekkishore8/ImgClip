...
print("Welcome! This script will clip multiple IMG files using a shapefile.")
print("Please follow the prompts to select your input and output locations.")

print("Step 1: Please select the folder containing your .IMG raster files.")
raster_folder <- tk_choose.dir(caption = "Select Folder with IMG Raster Files")
...
img_files <- list.files(path = raster_folder,
                        pattern = "\\.img$",
                        full.names = TRUE,
                        ignore.case = TRUE)

if (length(img_files) == 0) {
  stop(paste("No .IMG files found in the selected folder:", raster_folder, ". Please check the folder and file extensions."))
} else {
  print(paste("Found", length(img_files), "IMG files to process."))
  print("Files to be processed:")
  print(basename(img_files))
}
...
for (i in 1:length(img_files)) {
  ...
  print(paste0("----------------------------------------------------"))
  print(paste0("Processing file ", i, " of ", length(img_files), ": ", raster_name))
  ...
  tryCatch({
    ...
    terra::writeRaster(raster_final_clip, output_path, overwrite = TRUE, datatype = terra::datatype(current_raster), filetype = "ENVI")
